...
def fix_sequence(request):
    try:
        with connection.cursor() as cursor:
            ...
            # 2. Tables Django admin
            cursor.execute("SELECT setval('django_admin_log_id_seq', COALESCE((SELECT MAX(id) FROM django_admin_log), 1))")
            
            # 3. Tables auth
            cursor.execute("SELECT setval('auth_user_id_seq', COALESCE((SELECT MAX(id) FROM auth_user), 1))")
            cursor.execute("SELECT setval('auth_group_id_seq', COALESCE((SELECT MAX(id) FROM auth_group), 1))")
            cursor.execute("SELECT setval('auth_permission_id_seq', COALESCE((SELECT MAX(id) FROM auth_permission), 1))")
            
            # 4. Tables content types
            cursor.execute("SELECT setval('django_content_type_id_seq', COALESCE((SELECT MAX(id) FROM django_content_type), 1))")
            
            # 5. Tables migrations
            cursor.execute("SELECT setval('django_migrations_id_seq', COALESCE((SELECT MAX(id) FROM django_migrations), 1))")
            
            # 6. Tables gestion
            cursor.execute("SELECT setval('gestion_agent_id_seq', COALESCE((SELECT MAX(id) FROM gestion_agent), 1))")
            cursor.execute("SELECT setval('gestion_chauffeur_id_seq', COALESCE((SELECT MAX(id) FROM gestion_chauffeur), 1))")
            cursor.execute("SELECT setval('gestion_heuretransport_id_seq', COALESCE((SELECT MAX(id) FROM gestion_heuretransport), 1))")
            cursor.execute("SELECT setval('gestion_reservation_id_seq', COALESCE((SELECT MAX(id) FROM gestion_reservation), 1))")
            cursor.execute("SELECT setval('gestion_societe_id_seq', COALESCE((SELECT MAX(id) FROM gestion_societe), 1))")
            
            # 7. Tables mobile
            cursor.execute("SELECT setval('chauffeurs_mobile_mobilecoursestatus_id_seq', COALESCE((SELECT MAX(id) FROM chauffeurs_mobile_mobilecoursestatus), 1))")
            cursor.execute("SELECT setval('chauffeurs_mobile_mobilenotification_id_seq', COALESCE((SELECT MAX(id) FROM chauffeurs_mobile_mobilenotification), 1))")
            
        return HttpResponse("""
        ✅ TOUTES les séquences ont été corrigées !<br>
        - Affectations<br>
        - Courses<br>
        - Admin Logs<br>
        - Utilisateurs<br>
