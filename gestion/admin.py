...
class NotificationAdminAdmin(admin.ModelAdmin):
    list_display = ['titre', 'type', 'date_creation', 'lu', 'apercu_message']
    ...
    def apercu_message(self, obj):
        return obj.message[:100] + '...' if len(obj.message) > 100 else obj.message
    apercu_message.short_description = 'Message'
    
    actions = ['marquer_comme_lu']
    
    def marquer_comme_lu(self, request, queryset):
        for notification in queryset:
            notification.marquer_comme_lu(request.user)
