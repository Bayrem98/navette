...
def verifier_agent_dans_planning(agent, date_course, heure_course, type_transport, request):
    """
    Vérifie si un agent est dans le planning pour une date/heure donnée
    """
    try:
        ...
        if not gestionnaire.recharger_planning_depuis_session():
            ...
            return True  # Si pas de planning, pas de notification
        
        # Convertir la date en jour de semaine
        jours_fr = ['Lundi', 'Mardi', 'Mercredi', 'Jeudi', 'Vendredi', 'Samedi', 'Dimanche']
        date_course_str = date_course.strftime("%d/%m/%Y")
        
        # Trouver le jour correspondant dans le planning
        jour_correspondant = None
        for jour_planning, date_planning_str in gestionnaire.dates_par_jour.items():
            if date_planning_str == date_course_str:
                jour_correspondant = jour_planning
                break
        
        if not jour_correspondant:
            print(f"⚠️ Date {date_course_str} non trouvée dans le planning")
            return True
        ...
        form_filtre = FiltreFormPlanning(jour_correspondant, type_transport, heure_course)
        liste_transports = gestionnaire.traiter_donnees(form_filtre)
        
        # Vérifier si l'agent est dans la liste
        agent_nom = agent.nom.strip().lower()
        for transport in liste_transports:
            if transport.get('agent', '').strip().lower() == agent_nom:
                print(f"✅ Agent {agent.nom} PROGRAMMÉ pour {jour_correspondant} à {heure_course}h")
                return True
        
        print(f"❌ Agent {agent.nom} NON PROGRAMMÉ pour {jour_correspondant} à {heure_course}h")
        return False
...
def api_creer_course(request):
    ...
    try:
        ...
        print(f"🇫🇷 HEURE SERVEUR: {heure_actuelle}h")
        print(f"📅 DATE SERVEUR: {date_actuelle}")
...
# Remplace par un simple log
        ...
        if heure_actuelle >= 6:
            ...
            # Nuit (00h-05h) -> on est dans la journée d'hier
            date_hier = date_actuelle - timedelta(days=1)
            
            if date_obj == date_actuelle:
                print(f"🌙 Nuit - Redirection vers hier ({date_hier})")
                date_a_utiliser = date_hier
            elif date_obj == date_hier:
                print(f"✅ Date correspond à hier ({date_hier})")
                date_a_utiliser = date_hier
            else:
                return JsonResponse({
                    'success': False,
...
def api_agents_disponibles(request):
    """API pour voir les agents disponibles - VERSION CORRIGÉE"""
    chauffeur_id = request.session.get('chauffeur_id')
    ...
    try:
        ...
        # ========== CHARGER LE PLANNING ==========
        agents_programmes_par_heure = {}  # Dictionnaire: heure -> liste d'agents
        ...
        try:
            ...
            if gestionnaire.recharger_planning_depuis_session():
                ...
                if jour_correspondant:
                    # Récupérer TOUTES les heures pour ce type de transport
                    HeureTransport = apps.get_model('gestion', 'HeureTransport')
                    toutes_heures = HeureTransport.objects.filter(
                        type_transport=type_transport,
                        active=True
                    ).order_by('heure')
                    
                    for heure_transport in toutes_heures:
                        heure_valeur = heure_transport.heure
                        ...
                        agents_programmes_par_heure[heure_valeur] = [t['agent'].strip().lower() for t in liste_transports]
                    
                    planning_info = {"charge": True, "message": f"Planning chargé avec {len(agents_programmes_par_heure)} heures"}
                    print(f"✅ Planning chargé: {list(agents_programmes_par_heure.keys())}")
                    
        except Exception as e:
            print(f"⚠️ Erreur planning: {e}")
        
        # ========== RÉCUPÉRER TOUS LES AGENTS ==========
        # 🔥 IMPORTANT: On ne filtre PAS par occupation à cette heure
        # On veut voir TOUS les agents, y compris ceux déjà occupés
        ...
        for agent in tous_agents:
            # Vérifier à quelle heure l'agent est programmé
            heure_programmee = None
            est_programme = False
            
            for h, agents_progs in agents_programmes_par_heure.items():
                if agent.nom.strip().lower() in agents_progs:
                    heure_programmee = h
                    est_programme = True
                    break
            ...
            agent_data = {
                ...
                'heure_programmee': heure_programmee,  # 🔥 AJOUT: Heure de programmation
                'peut_reserver': True
            }
            ...
            if agent.id in reservations_par_agent:
                ...
                agent_data['est_reserve'] = False
            
            agents_list.append(agent_data)
        
        # ========== TRIER LES AGENTS ==========
        # 🔥 PRIORITÉ: Heure sélectionnée (heure_int) en premier
        agents_list.sort(key=lambda x: (
            0 if x.get('heure_programmee') == heure_int else 1,  # Heure choisie en 1er
            0 if x.get('est_programme') else 1,  # Programmés ensuite
            0 if x.get('est_reserve') else 1,  # Réservés ensuite
            x.get('nom', '')  # Ordre alphabétique
        ))
        
        # Statistiques
        total_agents = len(agents_list)
        agents_heure_choisie = len([a for a in agents_list if a.get('heure_programmee') == heure_int])
        agents_programmes = len([a for a in agents_list if a.get('est_programme')])
        agents_reserves = len([a for a in agents_list if a.get('est_reserve')])
        ...
        return JsonResponse({
            ...
            'stats': {
                'total': total_agents,
                'heure_choisie': agents_heure_choisie,
                'programmes': agents_programmes,
                'reserves': agents_reserves,
                'disponibles': total_agents - agents_reserves
            },
            'planning_info': planning_info,
            'date': date_a_utiliser.isoformat(),
            'heure_demandee': heure_int,
            'type_transport': type_transport,
...
def api_super_reservations_demain(request):
    ...
    try:
        ...
        # 3. Récupérer TOUS les chauffeurs pour les options de réservation
        tous_chauffeurs = Chauffeur.objects.filter(actif=True).order_by('nom')
        
        # 4. Préparer les données pour chaque heure
        heures_data = []
        ...
        def traiter_heure_avec_planning_data(heure_valeur, type_transport):
            ...
            for item in planning_data:
                ...
                jour_planning = jour_colonnes.get(jour_semaine, jour_semaine)
                horaire = item.get(jour_planning, '') or ''
...
def notify_admin_hors_planning(course, agent, chauffeur, request):
    """
    Crée une notification dans la base de données pour TOUS les admins
    """
    try:
        ...
        # Préparer les données
        date_str = course.date_reelle.strftime('%d/%m/%Y') if course.date_reelle else "date inconnue"
        heure_str = str(course.heure) if course.heure else "?"
        
        titre = f"🚨 Agent hors planning - {agent.nom}"
        message = (f"L'agent {agent.nom} a été transporté par {chauffeur.nom} "
                  f"le {date_str} à {heure_str}h alors qu'il n'était pas programmé.")
        lien = f"/admin/gestion/course/{course.id}/change/"
        
        # Créer UNE notification (visible par tous les admins)
        ...
        print(f"✅ Notification #{notification.id} créée pour les admins")
        print(f"   Titre: {titre}")
        print(f"   Destinataires: {', '.join([a.username for a in admins])}")
